...
var facebook = require("../functions/facebook");
var linkedin = require("../functions/linkedin");
module.exports = {
  main: function() {
    // Get all accounts that have not been updated in 2 days
    Account.find(
      { lastRenewed: { $lt: new Date() - 172800000 }, socialType: "facebook" },
      function(err, oldAccounts) {
        ...
        for (let index in arrayOfUniqueSocialIDs) {
          ...
          Account.find({ socialID: account.socialID }, function(
            err,
            accountsOfSameSocialID
          ) {
