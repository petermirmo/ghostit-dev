...
class WritersBrief extends Component {
  ...
  getWritersBriefs = () => {
    axios.get("api/writersBriefs").then(res => {
      ...
      for (let index in writersBriefs) {
        writersBriefs[index].cycleStartDate = new moment(
          writersBriefs[index].cycleStartDate
        );
        writersBriefs[index].cycleEndDate = new moment(
          writersBriefs[index].cycleEndDate
        );
      }
      ...
      if (this._ismounted) {
        this.setState({
          writersBriefs: writersBriefs,
          activeWritersBriefIndex: activeWritersBriefIndex
        });
      }
    });
  };
  ...
  render() {
    ...
    const adminManagerOrDemo =
      user.role === "admin" || user.role === "manager" || user.role === "demo";
    ...
    for (let index in writersBriefs) {
      ...
      if (activeWritersBrief) {
        if (
          writersBriefs[index].cycleStartDate ===
          activeWritersBrief.cycleStartDate
        ) {
          active = "active";
        }
      }
      ...
      <div>
        <div className="writers-brief-page-container">
          <div className="past-writers-brief-container">
            {adminManagerOrDemo && (
              <button
                className="new-writers-brief"
                onClick={() => this.createNewWritersBrief()}
              >
          ...
          {activeWritersBrief && (
            <WritersBriefForm
              writersBrief={activeWritersBrief}
              updateWritersBrief={this.updateWritersBrief}
            />
          )}
        </div>
      </div>
