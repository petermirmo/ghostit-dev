...
module.exports = {
	saveNewsletter: function(req, res) {
		...
		Newsletter.findOne({ _id: req.params.newsletterID }, async function(err, foundNewsletter) {
			if (err) {
				handleError(res, err);
				return;
			}
			...
			if (newsletterFile.localPath) {
				...
				await cloudinary.v2.uploader.upload(
					...
					function(error, result) {
						...
						newNewsletter.wordDoc = { url: result.url, publicID: result.public_id, name: newsletterFileName };
					}
				);
			}
	...
	deleteNewsletter(req, res) {
		Newsletter.findOne({ _id: req.params.newsletterID }, async function(err, newsletter) {
			if (err) {
				handleError(res, err);
			} else if (newsletter) {
				if (newsletter.userID === req.user._id || req.user.role === "admin" || req.user.role === "manager") {
